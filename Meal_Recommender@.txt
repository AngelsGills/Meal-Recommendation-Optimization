...
txt = input(prompt,"s");
if txt == 'Y'
    prompt = "Enter number for breakfast meal limit:";
    a = input(prompt);
else 
    a = 0;
end
prompt = "Do you want to include entrées? Y/N";
txt = input(prompt,"s");
if txt == 'Y'
    prompt = "Enter number for entrée meal limit:";
    b = input(prompt);
else 
    b = 0;
end
prompt = "Do you want to include protein? Y/N";
txt = input(prompt,"s");
if txt == 'Y'
    prompt = "Enter number for protein meal limit:";
    c = input(prompt);
else 
    c = 0;
end
...
txt = input(prompt,"s");
if txt == 'Y'
    prompt = "Enter number for side meal limit:";
    d = input(prompt);
else 
    d = 0;
end
...
txt = input(prompt,"s");
if txt == 'Y'
    prompt = "Enter number for soup meal limit:";
    e = input(prompt);
else 
    e = 0;
end
mealLimits = [a, b, c, d, e]; 			                            	% number of each mealType allowed (i.e., max) 
...
for i = 1:chromosome_length
    if selection(i) == 1
        ...
        message = sprintf('%s \n%s - %s - %s %s%s, %s %s%s, %s %s%s, %s %s%s, %s ', message,... 
            ...
            "Cal:",string(meal_table.Calories(i)),string(g),...
            "Protein:", string(meal_table.Protein(i)), string(g),...
            "Carbs:", string(meal_table.Carbs(i)), string(g),...
            "Fat:", string(meal_table.Fat(i)), string(g), string(x));
    end
end
